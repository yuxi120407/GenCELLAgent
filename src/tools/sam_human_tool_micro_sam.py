...
def sam_human_correction_ui(image_path: str, mask_path: str, save_path: str) -> str:
    ...
    # with col_sam:
    #     sam_clicked = st.button("Run SAM with Smart Prompts")
...
#             final_labels.extend([label_val] * len(pts))

# #         for x0, y0, x1, y1 in st.session_state.user_boxes:
# #             roi = binary_mask[y0:y1, x0:x1]
# #             roi_labeled = label(roi)
# #             if roi_labeled.max() > 0:
# #                 largest = np.argmax(np.bincount(roi_labeled.flat)[1:]) + 1
# #                 roi_mask = (roi_labeled == largest)
# #                 ys, xs = np.where(roi_mask)
# #                 for x_rel, y_rel in zip(xs, ys):
# #                     x_abs = x0 + x_rel
# #                     y_abs = y0 + y_rel
# #                     final_points.append([x_abs, y_abs])
# #                     final_labels.append(1)

# #         for label_val, poly_pts in st.session_state.user_polygons:
# #             xs, ys = zip(*poly_pts)
# #             rr, cc = polygon(ys, xs, shape=img_np.shape[:2])
# #             final_points.extend([[x, y] for x, y in zip(cc, rr)])
# #             final_labels.extend([label_val] * len(rr))

#         predictor.set_image(img_np)
#         masks, scores, _ = predictor.predict(
#             point_coords=np.array(final_points),
#             point_labels=np.array(final_labels),
#             multimask_output=True
#         )

#         best_mask = masks[np.argmax(scores)]
#         st.session_state.sam_mask = best_mask
    ...
    if "sam_mask" in st.session_state or "human_mask" in st.session_state:
        ...
        # if "sam_mask" in st.session_state and sam_clicked:
        #     overlay_sam = img_np.copy()
        #     overlay_sam[st.session_state.sam_mask > 0] = [255, 0, 0]
        #     blended_sam = cv2.addWeighted(img_np, 0.5, overlay_sam, 0.5, 0)
        #     with col2:
        #         st.image(blended_sam, caption="SAM + Smart Prompts", use_column_width=True)
