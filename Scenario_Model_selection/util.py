...
ROOT = "/home/idies/workspace/Temporary/xyu1/scratch/cellpose/"

EXPERIMENT_ROOT = "/home/idies/workspace/Temporary/xyu1/scratch/cellpose_results/"  # for scc
...
DATASETS = [
    # in-domain (LM)
    "tissuenet_new/one_chan", "tissuenet_new/multi_chan", "deepbacs", "PlantSeg_new/root", "livecell",
    "neurips-cell-seg/all", "neurips-cell-seg/tuning", "neurips-cell-seg/self",
    # out-of-domain (LM)
    "covid_if", "PlantSeg_new/ovules", "hpa", "lizard", "mouse-embryo", "ctc/hela_samples", "dynamicnuclearnet", "pannuke",
    # organelles (EM)
    #   - in-domain
...
def get_dataset_paths(dataset_name, split_choice):
    ...
    if len(split_names) > 1:
        ...
        if split_names[0] == "tissuenet_new":
            dataset_name = [split_names[0], "slices", split_names[1]]
        else:
            dataset_name = [split_names[0], split_names[1]]
    elif split_names[0] == "deepbacs":
...
def get_paths(dataset_name, split):
    ...
    if dataset_name == "PlantSeg_new/root" and split=="val":
        # Ensure image_paths and gt_paths are paired correctly
        paired = list(zip(image_paths, gt_paths))
        random.seed(42)
        sampled = random.sample(paired, 50)

        # Unzip back into two lists
        image_paths, gt_paths = zip(*sampled)

        # Convert back to lists (optional, but usually recommended)
        return list(image_paths), list(gt_paths)
    
    elif dataset_name == "PlantSeg_new/ovules" and split=="val":
        # Ensure image_paths and gt_paths are paired correctly
        paired = list(zip(image_paths, gt_paths))
        random.seed(42)
        sampled = random.sample(paired, 50)
